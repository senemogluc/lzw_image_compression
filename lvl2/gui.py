...
import  os
from    image_compression2  import lvl2, get_size

current_directory = os.path.dirname(os.path.realpath(__file__)) # Image Compression via LZW\lvl2
original_image_file_path = current_directory + '/squares.png'
temp_image_file_path = current_directory + '/result.png'
compressed_path = current_directory + '/compressed.bin'

def start():
    
    if os.path.exists("lvl2/compressed.bin"):
       os.remove("lvl2/compressed.bin")
    open("lvl2/compressed.bin", "x")
    ...
    oimg_title_label.grid(row = 1, column=0, columnspan=2)

    # Decompressed image title
    pimg_title_label = ttk.Label(frame, text="Decompressed Image")
    pimg_title_label.grid(row = 1, column=5, columnspan=2)
    ...
    oimg_size_label.grid(row = 3, column=0, columnspan=2)

    # Temporary image size
    pimg_size_label = ttk.Label(frame, text="Size of 'compressed.bin' is " + compressed_size + " bytes")
    pimg_size_label.grid(row = 3, column=5, columnspan=2)
    ...
    btn2['command'] = lambda:[lvl2(),modify_image(pimg_panel),modify_size(pimg_size_label)]
